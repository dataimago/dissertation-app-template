...
- **If `raw/manuscript/LOCKED.md` exists, it governs.** The manuscript beside it is the author's finished dissertation, imported as authoritative v1.0. Extend it — never regenerate, restructure, summarize-and-replace, or "improve" it. Legitimate work builds on top: derived papers, a revised edition as a new version, new chapters clearly marked post-v1.0. Seed the wiki and any thesis-source scaffolding *from* those files, and where they and the interview answers disagree, the files win. (This is stronger than the `raw/` rule above: that one forbids editing the bytes, this one forbids replacing the work.)
